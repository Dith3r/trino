...
public final class LazyResultBlock
        implements ValueBlock
{
    private static final int INSTANCE_SIZE = instanceSize(LazyResultBlock.class);
    ...
    static LazyResultBlock createLazyResultBlockInternal(int positionCount, Type type, @Nullable boolean[] rowIsNull, Block[] fieldBlocks)
    {
        ...
        return new LazyResultBlock(positionCount, type, rowIsNull, fieldBlocks, fixedSize);
    }
     ...
     */
    private LazyResultBlock(int positionCount, Type type, @Nullable boolean[] rowIsNull, Block[] fieldBlocks, int fixedSizePerRow)
    {
        if (positionCount < 0) {
    ...
    public static Block fromNotNullSuppressedFieldBlocks(int positionCount, Type type, Optional<boolean[]> rowIsNullOptional, Block[] fieldBlocks)
    {
        ...
        return createLazyResultBlockInternal(positionCount, type, rowIsNullOptional.orElse(null), fieldBlocks);
    }
    ...
    {
        return format("LazyResultBlock{fieldCount=%d, positionCount=%d}", fieldBlocks.length, positionCount);
    }
    ...
    public Block getLoadedBlock()
    {
        ...
        return new LazyResultBlock(positionCount, type, rowIsNull, loadedFieldBlocks, fixedSizePerRow);
    }

    @Override
    public LazyResultBlock copyWithAppendedNull()
    {
        ...
        return new LazyResultBlock(positionCount + 1, type, newRowIsNull, newBlocks, fixedSizePerRow);
    }

    @Override
    public String getEncodingName()
    {
        return LazyResultBlockEncoding.NAME;
    }

    @Override
    public LazyResultBlock copyPositions(int[] positions, int offset, int length)
    {
        ...
        return new LazyResultBlock(length, type, newRowIsNull, newBlocks, fixedSizePerRow);
    }

    @Override
    public LazyResultBlock getRegion(int positionOffset, int length)
    {
    ...
    @Override
    public LazyResultBlock copyRegion(int positionOffset, int length)
    {
        checkValidRegion(positionCount, positionOffset, length);
    ...
    public LazyResult getRow(int position)
    {
        checkReadablePosition(this, position);
        if (isNull(position)) {
            throw new IllegalStateException("Position is null");
        }
        return new LazyResult(position, type, fieldBlocks);
    }

    @Override
    public LazyResultBlock getSingleValueBlock(int position)
    {
        checkReadablePosition(this, position);
        ...
        return new LazyResultBlock(1, type, newRowIsNull, newBlocks, fixedSizePerRow);
    }
    ...
    public static List<Block> getRowFieldsFromBlock(Block block)
    {
        ...
        if (block instanceof RunLengthEncodedBlock runLengthEncodedBlock) {
            LazyResultBlock rowBlock = (LazyResultBlock) runLengthEncodedBlock.getValue();
        ...
        if (block instanceof DictionaryBlock dictionaryBlock) {
            LazyResultBlock rowBlock = (LazyResultBlock) dictionaryBlock.getDictionary();
            return rowBlock.fieldBlocksList.stream()
                    .map(dictionaryBlock::createProjection)
                    .toList();
        }
        if (block instanceof LazyResultBlock) {
            return ((LazyResultBlock) block).getFieldBlocks();
        }
    ...
    public static List<Block> getNullSuppressedRowFieldsFromBlock(Block block)
    {
        ...
        if (block instanceof LazyResultBlock rowBlock) {
